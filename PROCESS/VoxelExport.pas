...
uses CalcVoxelSliceThread, FileHandling, Math, Math3D, Calc, DivUtils, MAIN,
...
begin
    if not GUI.SizeOK(False) then Exit;
    ...
    begin
      PSLstart := Integer(@GUI.siLight5[0]);
...
var VoxCalcThreads: array of TFVoxelExportCalcThread;
    ...
    d: Double;
begin
  ThreadCount := Min(GUI.UpDownThread.Position, M3Vfile.VHeader.Height);
  try
    ...
    if Result then
    begin
      MCTparas.pSiLight := @GUI.siLight5[0];
  ...
  begin
    ...
    begin
      ...
      try
        ...
        VoxCalcThreads[x - 1].Priority        := cTPrio[GUI.ComboBoxThread.ItemIndex];
  ...
  else GUI.OutMessage('Error starting slice ' + IntToStr(nr));
end;
...
var VoxCalcThreads: array of TFVoxelExportCalcPreviewThread;
    ...
    d: Double;
begin
  ThreadCount := Min(GUI.UpDownThread.Position, PreviewSize);
  try
    M3Vfile.VHeader.TilingOptions := 0;
    bGetMCTPverbose := False;
    MCTparas := getMCTparasFromHeader(M3Vfile.VHeader, False);
...
begin
    ...
    begin
      ...
      begin
        ...
        Save1bitPNG(OutputFolder + VProjectName + IntToStrL(FileNumber, il) +'.png', GUI.Image_M_1.Picture.Bitmap);
      end;
      Inc(FileNumber);
      if FileNumber > M3Vfile.Zslices then //ready with all slices
      begin
        GUI.EnableButtons;
        GUI.OutMessage('Finished rendering slices.');
        Label13.Caption := '';
      end
...
begin
    ...
    begin
      ...
      for i := 0 to 5 do VHeader.PHCustomF[i] := @HybridCustoms[i];
      GUI.MakeHeader;
      AssignHeader(@VHeader, @GUI.MHeader);
...
begin
 ...
 //   GUI.bCalcTile := False;
    FileNumber := 1;
    ImageScale := 1;
    SetLength(GUI.siLight5, M3Vfile.VHeader.Width * M3Vfile.VHeader.Height);
    GUI.mSLoffset := M3Vfile.VHeader.Width * SizeOf(TsiLight5);
    GUI.MHeader.Width := M3Vfile.VHeader.Width;
    GUI.MHeader.Height := M3Vfile.VHeader.Height;
    SetImageSize;
    CalcPreview := False;
