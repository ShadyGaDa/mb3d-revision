...
uses FileHandling, MAIN, HeaderTrafos, DivUtils, Math, ImageProcess,
  BRInfoWindow;
//LightGUI
...
begin
    ...
    begin
      ...
      if GUI.OPD.Execute then
        AOm3iFile := GUI.OPD.FileName
      else
      begin
...
begin
    ...
    bUserChange := False;
    SetHeader;
    AssignHeader(@OrigHeader, @GUI.MHeader);
    NewProject;
    Import;
end;
...
begin
    OrigHeader.TilingOptions := 0;
 //   OrigHeader.bStereoMode := 0;
    AssignHeader(@GUI.MHeader, @OrigHeader);
    IniCFsFromHAddon(GUI.MHeader.PCFAddon, GUI.MHeader.PHCustomF);
    GUI.Authors[0] := '?';
    GUI.Authors[1] := '';
    GUI.SetEditsFromHeader;
    GUI.SetLightFromHeader(OrigHeader);{LIGHT}
    GUI.Caption := ProjectName;
    GUI.ParasChanged;
end;
...
begin
  ...
  //    MHeader.bStereoMode := 0;
      ...
      Edit_M_26.Text := IntToStr(BigRenderData.brJPEGqual);
      SaveThisTile := True;
      CalcMand(False);
    end;
end;
