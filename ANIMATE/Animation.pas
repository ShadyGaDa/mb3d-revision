...
uses
  ...
  Dialogs, ComCtrls, ExtCtrls, FileHandling, MAIN, Buttons, StdCtrls,
  HeaderTrafos, TypeDefinitions, Math3D;
  //LightGUI
type
  TKeyFrame = record
...
begin
    ...
    begin
      yDif  := Abs(Top - GUI.Top - GUI.Height);
      if yDif < 17 then
      begin
        xDif := Abs(Left - GUI.Left);
        if xDif < 17 then GUI.bAniFormStick := 1;
      end
      else GUI.bAniFormStick := 0;
    end;
...
begin
    ...
    if AniOption > 0 then
    begin
      GUI.UpDown_M_1.Enabled := False;
      GUI.SpeedButton_M_3.Enabled  := False;
      GUI.SpeedButton_M_13.Enabled := False;
      GUI.SpeedButton_M_5.Enabled  := False;
      GUI.SpeedButton_M_6.Enabled  := False;
    end;
...
begin
    ...
    isCalculating     := False;
    GUI.UpDown_M_1.Enabled := True;
    GUI.SpeedButton_M_3.Enabled  := True;
    GUI.SpeedButton_M_13.Enabled := True;
    GUI.SpeedButton_M_5.Enabled  := True;
    GUI.SpeedButton_M_6.Enabled  := True;
    SetScrollButton;
    Invalidate;
end;
...
begin
  ...
  //  KeyFrames[HeaderCount - 1].KFsmooth := SpinEdit3.Value;
    ...
    AssignHeader(@KeyFrames[HeaderCount - 1].HeaderParas, @GUI.MHeader);
...
procedure TAnimationForm.SpeedButton8Click(Sender: TObject);  //copy Paras from KF to MAIN
begin
    AssignHeader(@GUI.MHeader, @KeyFrames[KFposLUT[CurrentNr - 1]].HeaderParas);
    IniCFsFromHAddon(GUI.MHeader.PCFAddon, GUI.MHeader.PHCustomF);
    GUI.MHeader.Width := AniWidth * AniScale;
    GUI.MHeader.Height := AniHeight * AniScale;
    GUI.MHeader.bImageScale := AniScale;
    GUI.MHeader.bStereoMode := 0;
    GUI.Authors[0] := '?';
    GUI.Authors[1] := '';
    GUI.SetEditsFromHeader;
    GUI.SetLightFromHeader(GUI.MHeader);{LIGHT}
    GUI.Caption := 'Keyframe #' + IntToStr(CurrentNr);
    GUI.ParasChanged;
    SetFocus;
    //+paint2D?
end;
...
begin
   ...
   {   end
      ...
      AssignHeader(@GUI.MHeader, @InterpolHeader);
      IniCFsFromHAddon(GUI.MHeader.PCFaddon, GUI.MHeader.PHCustomF);
      GUI.MHeader.bSliceCalc  := 2;
      GUI.MHeader.bCalc3D     := Byte(AniCalc3D);
      GUI.MHeader.Width       := AniWidth  * AniScale;
      GUI.MHeader.Height      := AniHeight * AniScale;
      GUI.MHeader.bImageScale := AniScale;
      GUI.MHeader.sDEstop     := GUI.MHeader.sDEstop * AniScale;
      GUI.MHeader.TilingOptions := 0;
      GUI.MHeader.bStereoMode := StereoMode;
      AssignLightVal(@GUI.HeaderLightVals, @InterpolLightVals);
...
begin
 ...
 //   AssignHeader(@GUI.MHeader, @KeyFrames[KFposLUT[CurrentNr - 1]].HeaderParas);
    PTHeaderCustomAddon(GUI.MHeader.PCFAddon)^ := PTHeaderCustomAddon(pHeader.PCFAddon)^;
    FastMove(pHeader.dZstart, GUI.MHeader.dZstart, 72);
 //   GUI.MHeader.MinimumIterations := pHeader.MinimumIterations;
    FastMove(pHeader.bIsJulia, GUI.MHeader.bIsJulia, 33);
    FastMove(pHeader.hVGrads, GUI.MHeader.hVGrads, 72);
    IniCFsFromHAddon(GUI.MHeader.PCFAddon, GUI.MHeader.PHCustomF);
  //  GUI.MHeader.Width := AniWidth * AniScale;
  //  GUI.MHeader.Height := AniHeight * AniScale;
  //  GUI.MHeader.bImageScale := AniScale;
    ...
    GUI.SetEditsFromHeader;
 //   GUI.SetLightFromHeader(GUI.MHeader);{LIGHT}
    GUI.Caption := 'View of keyframe #' + IntToStr(CurrentNr);
    GUI.ParasChanged;
    SetFocus;
end;
