...
uses MAIN, Math, Forms, Controls, Types, CalcThread, SysUtils,
...
begin
  ...
  try
    ...
    SetLength(Buf, 0);
    GUI.Image_M_1.Picture.Bitmap.Modified := True;
    GUI.Image_M_1.Invalidate;
  finally
...
begin
    ...
    b := Sqr(ImageScale);
    a := b div 2;
    if EndYh >= GUI.Image_M_1.Picture.Bitmap.Height then
       EndYh := GUI.Image_M_1.Picture.Bitmap.Height - 1;
    wid := GUI.Image_M_1.Picture.Bitmap.Width;
    if (wid < 1) or (wid * GUI.Image_M_1.Picture.Bitmap.Height * b > Length(fullSizeImage)) then Exit;
...
begin
    ...
    begin
      ...
      ystart := 0;
      xstart := 0;
      add2 := GUI.MHeader.Width * 9;
      ww := GUI.MHeader.Width * ImageScale;
    end;
    wid := BMP.Width;
    for y := 0 to BMP.Height - 1 do
    begin
      PB := BMP.ScanLine[y];
      PW := @GUI.siLight5[(y + ystart) * ww + xstart];
      Inc(PW, 4); //Zpos word
...
begin
    ...
    begin
      ...
      Image_M_1.Picture.Bitmap.PixelFormat := pf32bit;
      repeat
        ...
        success := True;
        try
          Image_M_1.Picture.Bitmap.Width  := w div ImageScale;
          Image_M_1.Picture.Bitmap.Height := h div ImageScale;
          getI1BMPSLs;
      ...
      UpDown_M_1.Position := 11 - ImageScale;
      mFSIoffset := w * 4;
...
begin
    ...
    ThreadCount := Min(GUI.UpDownThread.Position, MHeight);
    ...
    begin
      GUI.OutMessage('Could not build wavelet levels, no ambient shadow calced.');
      Exit;
    end;
    ...
    begin
      ...
      try
        ...
        ascThreadT0[x].Priority        := cTPrio[GUI.ComboBoxThread.ItemIndex];
      ...
      try
        ...
        ascThread[x].Priority        := cTPrio[GUI.ComboBoxThread.ItemIndex];
...
begin
    Result := False;
    PCTS.cCalcTime           := GetTickCount;
    ThreadCount              := Min(4, GUI.SpinEdit1.Value);
