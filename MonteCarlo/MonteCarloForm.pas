...
uses
  ...
  Vcl.ImgList, System.ImageList, JvExControls, JvgLabel;
...
//    procedure CategoryPanel_MC_StatsCollapse(Sender: TObject);
//    procedure CategoryPanel_MC_StatsExpand(Sender: TObject);
...
begin
    ...
    TrackBar2.Position := (MCparas.Light.TBoptions shr 23) and $3F;
    TrackBar3.Position := MCparas.bMCSaturation and $7F;
    ...
    Button4.Enabled := GUI.btnCalc3D.Caption <> 'Stop';
...
begin
    ...
    MCparas.bMCSaturation := TrackBar3.Position;
    ...
    MCparas.Light.TBoptions := (MCparas.Light.TBoptions and $E07FFFFF) or (TrackBar2.Position shl 23);  //gamma/L
end;
...
begin
    ClientWidth := Panel4.Width + ScrollBoxMC.Width + MCparas.Width;
    ClientHeight := Panel1.Height + Panel2.Height + Panel4.Height + MCparas.Height;
end;
...
begin
    ...
    GUI.MakeHeader;
    IniHeaderPointers;
    AssignHeader(@MCparas, @GUI.MHeader);
    ...
    SetParas;
    FName := ChangeFileExtSave(GUI.Caption, '');
...
begin
    ...
    IniHeaderPointers;
    AssignHeader(@GUI.MHeader, @MCparas);
    GUI.MHeader.MClastY := 0;
    GUI.MHeader.TilingOptions := 0;
    GUI.MHeader.iReflectsCalcTime := 0;
    GUI.MHeader.bSSAO24BorderMirrorSize := 0;
    GUI.Authors := Authors;
...
begin
    ...
    begin
      ...
      FitImageSize;
      JvgLabel6.Caption := 'Parameters version: ' + ProgramVersionStr(MCparas.sM3dVersion);
      if Abs(M3dVersion - MCparas.sM3dVersion) > 0.0005 then
      begin
        JvgLabel6.Caption := JvgLabel6.Caption + #13#10 +
...
//        Timer3.Enabled := True;
      ...
      GUI.bOutMessageLoud := True;
      try
        LoadBackgroundPicT(@MCparas.Light);
      finally
        GUI.bOutMessageLoud := False;
      end;
...
begin
    JvgLabel6.Top := CategoryPanelGroup_MC.Height + 8;
    if Timer3.Tag > 0 then
    begin
      Timer3.Tag := Timer3.Tag - 1;
      if (Timer3.Tag and 1) = 0 then JvgLabel6.Font.Color := clWindowText
                                else JvgLabel6.Font.Color := clRed;
    end
    else
    begin
      JvgLabel6.Font.Color := clWindowText;
...
begin   //set categoryGroupheight
    CategoryPanelGroup_MC.Height := CategoryPanel1.Height + CategoryPanel2.Height
