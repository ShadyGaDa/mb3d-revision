...
uses FileHandling, DivUtils, MAIN, Math, UITypes, Forms, HeaderTrafos, Calc,
  MapSequences;
    ...
    begin
      ...
      try
       ...
       {   dTmp := -D7BtoDouble(Header.Light.Lights[x].LXpos);
        ...
        GUI.OutMessage('Error, could not create volumic light map.');
        Result := False;
      end;
      if not Result then Exit;

      ThreadCount := Min(GUI.UpDownThread.Position, VolumeLightMap.CubeSize);
      ...
      begin
        ...
        try
          ...
          CVLMThread[x - 1].Priority        := cTPrio[GUI.ComboBoxThread.ItemIndex];
      ...
      if Sequence <> nil then begin
        ...
        if (s<>'') and FileExists(s) then begin
          ...
          if Result then begin
            GUI.OutMessage('Loaded map <'+ExtractFilename(s)+'>');
          end;
        end;
        if not Result then begin
          GUI.OutMessage('Loading map <'+ExtractFilename(s)+'> failed');
...
begin
  ...
  try
    ...
    on E: Exception do
      GUI.OutMessage('Map: ' + ExtractFileName(FileName) + #13#10 + E.Message);
  end;
