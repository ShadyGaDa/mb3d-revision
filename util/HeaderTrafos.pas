...
uses Math, DivUtils, formulas, CustomFormulas, MAIN, SysUtils,
     Animation, Interpolation, PaintThread, Types, DOF, Tiling,
...
begin
    ...
    begin
      ...
      begin
        DEoption := -1;
        GUI.OutMessage('Error, mixing of dIFS with other formulas will not work!');
        Break;
      end
      else
      case DEoption of
    ...
    begin
      ...
      begin
        DEoption := -1;
        GUI.OutMessage('Error, no dIFS shape formula selected.');
      end
      else DEoption := 20;
    end;
  ...
  //    Result.bDoubleCheck := (iOptions and 2) > 0;
      ...
      begin
        ...
        if Result.DEoption in [20..22] then
          GUI.OutMessage('Error, dIFS do not work in interpolation hybrid.')
     ...
     //   if (Result.DEoption <> 20) and bDisableADE then Result.IsCustomDE := False;
       ...
       //   if (Result.DEoption2 <> 20) and bDisableADE then Result.IsCustomDE2 := False;
          ...
          begin
            ...
            begin
              GUI.OutMessage('Error, second hybrid type must be dIFS.');
      ...
      begin
        ...
        begin
          bMCTisValid := False;
          GUI.OutMessage('Error, CPU must support SSE2 for dIFS.');
          Exit;
        end;
        if bMCTisValid then
        begin
   ...
   {    //   if OTrapMode then
          ...
          begin
            ...
            try
              ...
              if bGetMCTPverbose then GUI.OutMessage('Error in calcmap init.');
            end;
            if bGetMCTPverbose then GUI.OutMessage('Parameters ok.');
          end;
        end;
        if not bMCTisValid then GUI.OutMessage('Error, formula option is not valid.');
      end;
    end;
end;
...
begin
  ...
  //    iBGpicAndDivOptions := ((HLight.AdditionalOptions and 8) shr 3) or ((HLight.Lights[0].FreeByte and 1) shl 1) or
    //                 (HLight.AdditionalOptions and 32);
      ...
      DiffColLightMap := @GUI.DiffColMap;{LIGHT}
      DCLMapRotSin := 0;
      DCLMapRotCos := 1;
      DCLMapOffX := 0;
      DCLMapOffY := 0;
      ...
      begin              // 8
        ...
        LLightMaps[i] := @GUI.LightMaps[i];{LIGHT}
...
begin
    ...
    begin
      ...
      pBGRotMatrix := @GUI.BGpicRotMatrix;{NAVI}
      if iBackBMP <> 0 then
      with M3DBackGroundPic do
      begin
        if LMWidth > 0 then
